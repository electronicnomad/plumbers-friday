...
파일은 이유없이 디렉토리와 묶어야 될 것 같지만, 따지자면 디렉토리도 시스템에서는 파일과 별반 다를 것이 없기에
'파일과 권한 (file and permissions)'을 묶어 하나의 챕터를 만들었습니다.
...
DOS와 이것의 전통을 이어받은 운영체제에서는 (OS/2, Windows) 확장자가 결정적인 역할을 합니다. 즉, .exe 혹은
.com .bat 라는 확장자를 가져야 실행이 가능하고, .txt .pic 과 같은 확장자를 가지는 파일은
실제로 실행할 수 있는 형식을 갖추어도 실행될 수가 없습니다.

### 애플리케이션

사실 이것은 운영체제의 범위를 벗어나면 또 다른 유용함이 있는데, 바로 웹 서버 혹은 데몬과 같은 애플리케이션에서 효용이 나타납니다.
웹 서버는 서비스에 필요한 파일들을 이 확장자를 기준으로 하여 그 유형을 구분합니다. 그리고 클라이언트, 즉 웹 브라우저에 전송하는 데이터가 어떤 성격인지 알려 주고, 웹 브라우저는 그것을 기준으로 해석하게 됩니다.
.html .cgi .jpg .png .gif 등이 대표적이라고 할 수 있겠습니다.
...
그렇다면, 확장자가 파일의 성격을 대변하지 않는다면, 어떻게 그 파일이 어떤 역할을 혹은 어떤 형식인지 알 수 있는가?
라고 질문을 한다면, `file`이라는 명령어로 확인해 보는 방법이 있다고 답할 수 있습니다.
...
`/usr/bin/yes` 파일을 명령어 `file`로 조회한 것입니다. 그 결과는 아주 많은 정보를 가지고 있는데,
실행가능(executable)한 파일이라는 것을 알 수 있습니다. 나머지 정보는 여러분께서 지식을 쌓아가면서
하나씩 알게 되실 겁니다.

`/etc/hosts` 파일을 같은 방식으로 조회했는데, ASCII text라고 결과를 볼 수 있습니다.
...
이를 팔진법으로 표기하면 세 번째 열이 됩니다. 이것을 user + group + other로 엮어 사용하게 됩니다.

흔희 사용하게 되는 경우는 다음의 몇 가지 정도입니다.
